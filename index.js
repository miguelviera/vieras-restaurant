  ...
  try {
    ...
    if (req.cookies && req.cookies.token_mama) {
      ...
      if (data[decoded.email]) {
        res
          .status(200)
          .send({ msg: "Auth successful", data: data[decoded.email] });
      } else {
        res
          .status(500)
          .send({ msg: "Auth failed... there is no user with this token" });
      }
    } else {
...
app.get("/api/user/login/:email/:password/:remember", async (req, res) => {
  try {
    ...
    if (data[email] && data[email].password === password) {
      ...
      res.status(500).send({
        msg: "Login failed... Either email or password are incorrect",
      });
    }
  } catch (e) {
    res.status(500).send({ msg: e.message });
...
app.post("/api/user/signup", async (req, res) => {
  try {
    ...
    if (!data[email]) {
      ...
      res
        .status(500)
        .send({ msg: `The user ${email}, is already signed up...` });
    }
  } catch (e) {
    res.status(500).send({ msg: e.message });
  }
...
app.delete("/api/user/:email/:password", async (req, res) => {
  try {
    ...
    if (data[email] && data[email].password === password) {
      ...
      res
        .status(200)
        .send({ msg: "User was deleted successfully", data: data[email] });
    } else {
...
//Delete order
app.get("/api/user/:email/order/:orderId", async (req, res) => {
  try {
    const email = req.params.email;
    const orderId = req.params.orderId;
    const data = require("./data");
    delete data[email].orders[orderId];
    await writeFileAsync("./data.json", JSON.stringify(data));
    res
      .status(200)
      .send({ email: req.params.email, orderId: req.params.orderId });
  } catch (e) {
    res.status(500).send({ msg: e.message });
  }
});
...
app.get("/api/gallery", async (req, res) => {
  try {
    ...
    for (let i in images) {
      let buffer = await readFileAsync("./server_assets/" + images[i], {
        encoding: "base64",
      });
