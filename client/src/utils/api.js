...
export async function removeOrder(email, orderId) {
  const res = await fetch(`/api/user/${email}/order/${orderId}`);
  const json = await res.json();
  if (res.status === 200) {
    return json;
  } else {
...
export async function getAdminData(email, orderId) {
  // if (loggedIn) {
  const res = await fetch(`/api/admin/data1/${email}`);
  const json = await res.json();
  if (res.status === 200) {
    return json;
  } else {
...
export async function postNewOrder(email, payment, items, total, totalPrice) {
  ...
  if (res.status === 200) {
    alert(
      `Your order has been received by the system.\n Order ID: ${json.orderId}`
    );
    return json;
  } else {
    alert(json.msg);
